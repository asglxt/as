...
export async function seedBase(app: Awaited<ReturnType<typeof setupApp>>) {
  await app.pool.query(
    'TRUNCATE parent_bindings, import_jobs, audit_logs, payments, refunds, account_transactions, student_accounts, order_items, orders, fee_items, homework_records, homework, teaching_comments, comment_templates, attendance_records, teaching_logs, hour_transactions, enrollments, student_scores, exam_projects, exams, schedules, classrooms, lesson_upgrades, lessons, lesson_categories, subjects, class_students, classes, users, students, campuses RESTART IDENTITY CASCADE'
  );
