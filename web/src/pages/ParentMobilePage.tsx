...
function ParentScores({ studentId }: { studentId: number }) {
  ...
  return <section className="tm-page"><div className="tm-page-title"><h1>成绩报告</h1><p>{report.student.name}的成绩记录</p></div><ScoreAnalyticsPanel studentId={studentId} /><div className="pm-score-list">{report.scores.map((item: any) => <div className="pm-score-card" key={item.id}><div><span>{item.project_name} · {item.exam_name}</span><h3>{item.class_name ?? '未关联班级'}</h3><small>{String(item.exam_date).slice(0, 10)} · {item.source_path ?? item.source}</small></div><strong>{item.score ?? '-'}</strong>{item.remark && <p>{item.remark}</p>}</div>)}{report.scores.length === 0 && <div className="tm-empty">暂无成绩记录</div>}</div></section>;
}
