...
const SOURCE_SERIES = [
  { key: 'institution', name: '机构内', color: '#2563eb' },
  { key: 'school', name: '学校内', color: '#0f9f6e' },
  { key: 'third_party', name: '其他第三方', color: '#f59e0b' },
  { key: 'other', name: '其他', color: '#8b5cf6' }
];

function availableSourceSeries(history: any[]) {
  return SOURCE_SERIES.filter((source) => history.some((item) => (item.source_parent_slug ?? 'other') === source.key));
}
...
function GrowthChart({ history }: { history: any[] }) {
  ...
  useEffect(() => {
    ...
    const dates = [...new Set(history.map((item) => String(item.exam_date).slice(0, 10)))].sort();
    const grouped = new Map<string, Map<string, number[]>>();
    for (const item of history) {
      const sourceKey = item.source_parent_slug ?? 'other';
      const date = String(item.exam_date).slice(0, 10);
      if (!grouped.has(sourceKey)) grouped.set(sourceKey, new Map());
      const byDate = grouped.get(sourceKey)!;
      if (!byDate.has(date)) byDate.set(date, []);
      byDate.get(date)!.push(Number(item.numeric_score));
    }
    const series = SOURCE_SERIES.flatMap((source) => {
      const byDate = grouped.get(source.key);
      if (!byDate) return [];
      return [{
        name: source.name,
        type: 'line',
        ...
        lineStyle: { width: 3, color: source.color },
    ...
    chart.setOption({
      ...
      xAxis: { type: 'category', boundaryGap: false, data: dates.map((date) => date.slice(5)), axisLine: { lineStyle: { color: '#dfe6f0' } }, axisLabel: { color: '#7b879a', fontSize: 10 } },
...
export default function ScoreAnalyticsPanel({ studentId }: { studentId: number }) {
  ...
  const sources = availableSourceSeries(data.history);
  return (
    <div className="score-analytics">
      ...
      <section className="panel">
        <div className="panel-header"><h2>成绩成长曲线</h2><span className="subtitle">按来源拆分 · 平均 {summary.average ?? '-'} · 最高 {summary.best_score ?? '-'}</span></div>
        {sources.length > 0 && <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, margin: '2px 0 10px', fontSize: 12, color: '#5d6b7f' }}>{sources.map((source) => <span key={source.key}><i style={{ display: 'inline-block', width: 9, height: 9, borderRadius: '50%', background: source.color, marginRight: 5 }} />{source.name}</span>)}</div>}
        {data.history.length ? <GrowthChart history={data.history} /> : <p className="subtitle">暂无足够成绩记录</p>}
      </section>
